...
class CinemaHallsPrice extends React.Component {
    constructor(props) {
        super(props);
    ...
    renderCinemaHall(cinema_hall, index, cinemaHallsPrice ) {
        return(
            <li key={`cinema-hall-price-tab-${cinema_hall.id}`}>
                <input type="radio" className="conf-step__radio" name="chairs-hall-price" value={index} checked={index === this.state.selected_index} onChange={event=>{ this.setState({selected_index: parseInt(event.target.value)}); }} />
